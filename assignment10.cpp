...
// defined a separate class which stores variables in the form of integers so helps in evaluation 
class Stackforeval 
{
    int top;
    public:
...
int Stackforeval::gettop()
{
    return a[top];
} 

bool Stackforeval::push(int x)
{
    if (top >= (MAX - 1)) {
        cout << "Stack in Overflow";
        return false;
...
bool Stackforeval::isEmpty()
{
    return (top < 0);
}

bool Stackforeval::isFull()
{
    return (top == MAX-1);
}
